...
粘贴原文时可以在输入栏里填几个关键词，比如 `面试、报销、3 号楼`。它解决的是一个
通用摘要做不到的问题：**什么信息对你重要，光看文本是推不出来的**。

它怎么起作用：

1. 关键词会随原文一起交给模型，要求优先覆盖它们；
2. 服务端再做一次**确定性校验**——原文提到了某个关键词、但要点里一条都没覆盖到，
   就从原文里把那个句子抽出来补成一条要点，并标注「（关注点）」；
3. 卡片上把命中的关键词高亮，卡头显示「含你关注的：报销」。

第 2 步是这个功能真正的价值：它让「AI 漏掉你在意的信息」从看模型心情变成**可断言的保证**，
也是唯一能验证摘要完整性的手段。

两个刻意的设计：

- **它是一次性的**，只影响这一次生成，生成完输入框就清空，不进入任何长期配置——
  所以不会膨胀、不需要治理、也不会变成分类功能的入口。
- **原文没提到的关键词不会被补进来**。不算"遗漏"，也不会为了凑数编造内容。
- 单张卡最多 20 个关注点。上限的作用是逼你取舍，20 个以上的"关注点"等于没有关注点。

关注点会随卡片一起存下来，这样日后回看能知道**当时在关注什么**。存的是记录，不是配置。
...
关注点**不做**全局关键词库、权重、正则、按关键词分组或生成多张卡——它只在单张卡内加权与标注。
