...
function summarize(rawText: string, keywords: string[] = []) {
...
describe('createMockSummarizer', () => {
  it('标记产出者为 mock', async () => {
    const result = await summarize(NOTICE);
    assert.equal(result.provider, 'mock');
  });

  it('从方括号抬头识别来源', async () => {
    const { draft } = await summarize(NOTICE);
    assert.equal(draft.source, '教务处');
  });

  it('识别 "来源：" 形式的来源', async () => {
    const { draft } = await summarize('来源：后勤处\n停水通知：明天上午停水三小时。');
  ...
  it('"前" 后不接动作词时不当作截止语义', async () => {
    const { draft } = await summarize('请于3月5日前台签到并领取材料。');
    assert.equal(draft.time, '3月5日');
  });

  it('"前提" 等复合词不会被误判为截止', async () => {
    const { draft } = await summarize('会议的前提条件是完成报名，时间为3月5日14:00。');
    assert.equal(draft.time, '3月5日14:00');
  });

  it('没有截止语义时取第一个出现的时间', async () => {
    const { draft } = await summarize('会议定于3月5日14:00在第三会议室召开。');
    assert.equal(draft.time, '3月5日14:00');
  });

  it('没有日期时时间为 null', async () => {
    const { draft } = await summarize('请大家记得把门关好，注意安全用电。');
  ...
  it('剥掉 Markdown 强调符与项目符号', async () => {
    const { draft } = await summarize('- **重要通知**：明天上午停水三小时，请提前储水。');
    assert.equal(draft.title, '重要通知：明天上午停水三小时，请提前储水');
  });

  it('整句都是装饰符号时不会产出空标题', async () => {
    const { draft } = await summarize('❗️❗️❗️❗️\n请于3月8日前提交报名表，逾期不再受理。');
  ...
  it('过滤掉寒暄类噪声行', async () => {
    const { draft } = await summarize('收到\n谢谢\n请于3月8日前提交报名表，联系人张老师。');
...
describe('createMockSummarizer 对关注点的处理', () => {
  it('命中关注点的句子被显著提权', async () => {
    // 这句话本身只有"报"这类弱信号，正常会被下面那句强信号压住
    const rawText = '报销材料请交到财务处。\n请于3月8日24:00前提交报名表，逾期不再受理。';

    const withoutKeywords = await summarize(rawText);
    const withKeywords = await summarize(rawText, ['报销']);

    assert.equal(
      withoutKeywords.draft.keyPoints[0]?.includes('3月8日'),
      true,
      '没有关注点时，强信号句应排第一',
    );
    assert.equal(
      withKeywords.draft.keyPoints[0]?.includes('报销'),
      true,
      '有关注点「报销」时，它应当排到第一',
    );
  });

  it('完全不相关的关注点不影响排序', async () => {
    const rawText = '请于3月8日24:00前提交报名表，逾期不再受理。';
    const { draft } = await summarize(rawText, ['量子力学']);
    assert.match(draft.keyPoints[0] ?? '', /3月8日/);
  });

  it('关注点不会凭空造出原文没有的要点', async () => {
    const { draft } = await summarize('明天上午停水三小时，请提前储水。', ['报销', '面试']);
    assert.ok(!draft.keyPoints.some((point) => point.includes('报销')));
    assert.ok(!draft.keyPoints.some((point) => point.includes('面试')));
  });
});
