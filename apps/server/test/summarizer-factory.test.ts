...
describe('createSummarizer', () => {
  it('没有 Key 时返回 mock 摘要器', async () => {
    ...
    const result = await summarizer.summarize('请于3月8日前提交报名表，联系人张老师。');
  ...
  it('空字符串 Key 也视为未配置', async () => {
    ...
    const result = await summarizer.summarize('明天上午停水三小时，请提前储水。');
    assert.equal(result.provider, 'mock');
  });
  ...
  it('有 Key 时走 DeepSeek，并把请求发到指定端点', async () => {
    ...
    const result = await summarizer.summarize('随便一段通知');
